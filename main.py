...
def process_csv_uploads(files_data: list, out_dir: str) -> dict:
    ...
    def norm(val):
        return "__null__" if val is None else str(val).strip().lower()

    def get_hex(existing):
        i = 1
        while True:
            cid = f"{i:08x}"
    ...
    for item in files_data:
        ...
        for ri, row in enumerate(rows):
            if not row:
                continue
            tr_key = (table_name, ri)
            if tr_key not in tr_to_id:
                new_tr_id = get_hex(set(tr_to_id.values()))
                tr_to_id[tr_key] = new_tr_id
                id_to_tr[new_tr_id] = tr_key
            tr_id = tr_to_id[tr_key]

            for ci, val in enumerate(row):
                if ci < len(headers):
                    col = headers[ci]
                    norm_val = norm(val)
                    tcv_key = (table_name, col, norm_val)
                    if tcv_key not in tcv_to_id:
                        new_tcv_id = get_hex(set(tcv_to_id.values()))
                        tcv_to_id[tcv_key] = new_tcv_id
                        id_to_tcv[new_tcv_id] = tcv_key
                    tcv_id = tcv_to_id[tcv_key]
    ...
    # Write tcv_to_id.csv
    with (out / "tcv_to_id.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["table", "column", "value", "id"])
        for (t, c, v), i in sorted(tcv_to_id.items(), key=lambda x: x[1]):
            w.writerow([t, c, v, i])

    # Write id_to_tr.csv
    with (out / "id_to_tr.csv").open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["id", "table", "row_id"])
        for i, (t, r) in sorted(id_to_tr.items(), key=lambda x: x[0]):
            w.writerow([i, t, r])

    # Write inverted_index.csv
    with (out / "inverted_index.csv").open("w", encoding="utf-8") as f:
        for tcv_id, tr_ids in sorted(index.items()):
            f.write(tcv_id + "," + ",".join(sorted(tr_ids)) + ", \n")

    # Write db6k.dat
    with (out / "db6k.dat").open("w", encoding="utf-8") as f:
        for tcv_id, tr_ids in sorted(index.items()):
            f.write(",".join([tcv_id] + sorted(tr_ids)) + "\n")

    schema = {pt["table"]: pt["headers"] for pt in processed_tables}
    return {
        "tables": processed_tables,
        "schema": schema,
        "total_postings": total_postings,
        "unique_tcv": len(tcv_to_id),
        "unique_tr": len(tr_to_id),
